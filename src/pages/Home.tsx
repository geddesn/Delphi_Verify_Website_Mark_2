    ...
    <>
      ...
      <Section tone="inverse" className="relative overflow-hidden">
        ...
        <Container className="relative z-10">
          {/* [&>*]:min-w-0 and minmax(0,…) are both load-bearing.

              Grid items default to min-width:auto, so an `auto` track cannot
              size below its content's min-content width. The certificate
              panel contains a 48-character evidence hash set to nowrap by
              `truncate`, whose min-content is its full 371px — which forced
              this single mobile column to 490px inside a 313px container,
              dragged the headline out with it, and let the section's
              overflow-hidden clip "Trusted evidence for the physical world"
              mid-word on a phone.

              min-w-0 on the items lets the track shrink, which is also what
              finally allows `truncate` to render an ellipsis rather than
              silently overflow. */}
          <div className="grid items-center gap-14 [&>*]:min-w-0 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] lg:gap-20">
